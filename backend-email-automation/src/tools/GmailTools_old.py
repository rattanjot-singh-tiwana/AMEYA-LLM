...
class GmailToolsClass:
    ...
    def fetch_unanswered_emails(self, hours=24, max_results=50):
        """
        ...
        try:
            ...
            recent_emails = self.fetch_recent_emails(hours=hours, max_results=max_results)
    ...
    def fetch_recent_emails(self, hours=24, max_results=500):
        try:
            # Set delay based on provided hours
            now = datetime.now()
            delay = now - timedelta(hours=hours)

            # Format for Gmail query using RFC3339 timestamp
    ...
    def _create_html_email_message(self, recipient, subject, reply_text):
        """Creates a simple HTML email message with proper formatting."""
